...
public class FeedReadController {
    ...
    /**
     * 특정 사용자의 피드 목록 조회 (FD-803)
     * 
     * @param userId 사용자 ID
     * @param feedType 피드 타입 (DAILY, EVENT, RANKING) - 선택사항
     * @param page 페이지 번호 (기본값: 0)
     * @param size 페이지 크기 (기본값: 20)
     * @param sort 정렬 기준 (latest, popular)
    ...
    public ResponseEntity<ApiResponse<PaginatedResponse<FeedListResponseDto>>> getUserFeeds(
        ...
        log.info("사용자 피드 목록 조회 요청 - userId: {}, feedType: {}, page: {}, size: {}, sort: {}", 
        ...
        if (type != null) {
            feedPage = feedReadService.getUserFeedsByType(userId, type, pageable, userDetails);
        } else {
            feedPage = feedReadService.getUserFeeds(userId, pageable, userDetails);
        }
        ...
        log.info("사용자 피드 목록 조회 완료 - userId: {}, feedType: {}, 총 {}개, 현재 페이지 {}개", 
