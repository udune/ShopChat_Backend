...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    // 내 가게에서 주문한 주문을 모두 찾는다. 목록 조회 (주문일 기준 최신순으로 정렬한다)
    @EntityGraph(attributePaths = {"orderItems", "orderItems.productOption", "orderItems.productOption.product", "orderItems.productImage"})
    Page<Order> findByOrderItemsProductOptionProductStoreSellerIdOrderByCreatedAtDesc(Long sellerId, Pageable pageable);

    // 내 가게에서 주문한 주문 + 특정 주문 상태인 것만 필터링해서 모두 찾는다. 목록 조회 (주문일 기준 최신순으로 정렬한다)
    @EntityGraph(attributePaths = {"orderItems", "orderItems.productOption", "orderItems.productOption.product", "orderItems.productImage"})
